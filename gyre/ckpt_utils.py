...
# Derived from https://github.com/huggingface/diffusers/blob/main/scripts/convert_original_stable_diffusion_to_diffusers.py
# Original copyright notice from that file below
...
def load_as_models(
    ...
    if not os.path.isfile(config):
        config = os.path.join(GYRE_BASE_PATH, "ldm_config", config + ".yaml")

    if not os.path.isfile(config):
        raise RuntimeError(
            f"Config {original_config} doesn't exist and isn't one of the included"
        )
    ...
    if (
        ...
        image_size = 768 if image_size is None else image_size
    else:
    ...
    if model_type == "FrozenOpenCLIPEmbedder":
        ...
        if should_load("text_encoder") or should_load("tokenizer"):
            raise RuntimeError(f"Unknown text_encoder model type {model_type}")
...
def load_as_pipeline(
    ...
    **kwargs,
    ...
    models = load_as_models(
        config,
        ckpt_path=ckpt_path,
        safetensors_path=safetensors_path,
        image_size=image_size,
        extract_ema=extract_ema,
        blacklist=set(kwargs.keys()),
    )

    extra = {}
    if "requires_safety_checker" not in kwargs:
        extra["requires_safety_checker"] = (
            "safety_checker" in kwargs or "safety_checker" in models
        )

    return StableDiffusionPipeline(**kwargs, **models, **extra)
